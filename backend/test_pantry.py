...
def preparar_produto_para_db(api_data):
    ...
    return {
        ...
        # CORREÇÃO: Converter para int, pois a coluna DB é bigint (int8)
        "weight": int(float(api_data.get("product_quantity", 0))), 
...
def teste_com_scanner_real():
    ...
    dados_api_printados = json.dumps(dados_api, indent=2, ensure_ascii=False)
    with open("ultimo_produto_api.json", "w", encoding="utf-8") as f:
        f.write(dados_api_printados)
